...
export default function HabitCard({ habit, done, completions, freezes, onToggle, onFreeze, overdue }) {
  ...
  return (
    <div className={`group relative flex w-full items-center gap-4 rounded-2xl border p-4 transition-all duration-200 ${
      done || frozenToday
        ? 'border-neutral-100 bg-neutral-50 opacity-60 dark:border-neutral-900 dark:bg-neutral-900/60'
        : 'border-neutral-200 bg-white dark:border-neutral-800 dark:bg-[#111]'
    }`}>
      <button onClick={onToggle} className="flex flex-1 items-center gap-4 text-left hover:opacity-80 transition-opacity">
        {/* checkbox */}
        <span
        ...
        <span className="min-w-0 flex-1">
          ...
          {frozenToday && !done && (
            <span className="text-xs font-medium text-blue-400 dark:text-blue-500">Streak frozen ❄️</span>
          )}
          {habit.time && !frozenToday && (
      ...
      {canFreeze && onFreeze && (
        <button
          ...
          title="Freeze streak (1 per month — life happens)"
          className="shrink-0 rounded-xl border border-neutral-200 px-2.5 py-1.5 text-xs font-bold text-neutral-400 transition hover:border-blue-300 hover:text-blue-500 dark:border-neutral-700 dark:text-neutral-500 dark:hover:border-blue-600 dark:hover:text-blue-400"
        >
          ❄️
        </button>
      )}
