...
function TodayTasks({ dayKey, tasks, goals, habits, recurringTasks, onAddTask, onToggleTask, onUpdateTask, onDeleteTask, onAddRecurring, onDeleteRecurring }) {
  ...
  function add() {
    ...
    onAddTask(activeKey, text.trim(), goalId, priority, habitId, extra)
    setText('')
    setTaskTime('')
    setTaskDuration(1)
  }
    ...
    <section className="rounded-3xl border border-neutral-200 p-6 dark:border-neutral-800 dark:bg-[#111]">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          <h3 className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-400 dark:text-neutral-500 shrink-0">
            Tasks {list.length > 0 && <span className="ml-1 text-neutral-300 dark:text-neutral-600">{doneCount}/{list.length}</span>}
          </h3>
          {view === 'list' && (
            <div className="flex items-center gap-1">
              <button onClick={() => setDate((d) => addDays(d, -1))} title="Previous day" className="flex h-6 w-6 items-center justify-center rounded-md border border-neutral-200 text-neutral-500 transition hover:border-neutral-400 dark:border-neutral-700 dark:text-neutral-400 dark:hover:border-neutral-500 text-sm">‹</button>
              <span className="text-xs font-semibold text-neutral-600 dark:text-neutral-400 whitespace-nowrap">
                {isToday ? 'Today' : dayLabel}
              </span>
              <button onClick={() => setDate((d) => addDays(d, 1))} title="Next day" className="flex h-6 w-6 items-center justify-center rounded-md border border-neutral-200 text-neutral-500 transition hover:border-neutral-400 dark:border-neutral-700 dark:text-neutral-400 dark:hover:border-neutral-500 text-sm">›</button>
              {!isToday && (
                <button onClick={() => setDate(new Date())} className="rounded-md border border-neutral-200 px-2 py-0.5 text-[10px] font-bold text-neutral-400 transition hover:border-neutral-400 dark:border-neutral-700 dark:text-neutral-500 dark:hover:border-neutral-500">Today</button>
              )}
            </div>
          )}
        </div>
          ...
          return (
            <div key={t.id} className="flex flex-wrap sm:flex-nowrap items-center gap-2 border-b border-neutral-100 pb-2 last:border-b-0 last:pb-0 dark:border-neutral-900/50">
              <div className="flex flex-1 items-center gap-2 min-w-0">
                <PriorityDot priority={t.priority} onClick={() => onUpdateTask(activeKey, t.id, { priority: nextPriority(t.priority || 'medium') })} />
                <button
                  onClick={() => onToggleTask(activeKey, t.id)}
              ...
              <div className="flex items-center gap-1.5 ml-7 sm:ml-0">
                ...
                <Select
                  ...
                  onChange={(e) => onUpdateTask(activeKey, t.id, { time: e.target.value === '' ? null : `${pad(Number(e.target.value))}:00` })}
                  title="Set a time"
                  compact
                ...
                {t.time && (
                  <Select
                    value={t.duration || 1}
                    onChange={(e) => onUpdateTask(activeKey, t.id, { duration: Number(e.target.value) })}
                    title="Set duration"
                    compact
                ...
                {(goals.length > 0 || habits.length > 0) && (
                  <LinkSelect
                    ...
                    onChange={(e) => {
                      ...
                      onUpdateTask(activeKey, t.id, { goalId, habitId })
                    }}
                    compact
                ...
                <button
                  onClick={() => onDeleteTask(activeKey, t.id)}
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-neutral-100 pt-4 dark:border-neutral-900/50">
        ...
        <input
          ...
          placeholder={isToday ? 'Add a task for today…' : `Add a task for ${dayLabel}…`}
